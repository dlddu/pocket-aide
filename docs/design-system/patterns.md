...
## 9. 편집 시트 패턴 (Edit Sheet)

원본 화면 위에 시트 컴포넌트가 일시적으로 떠올라 단일 결정(우선순위 설정, 카테고리 선택 등)을 받고 닫히는 흐름.

### 흐름
1. **트리거**: 명시적 사용자 액션 (항목 추가 직후 자동 노출, 카드 탭, 길게 누르기→메뉴 등)
2. **시트 노출**: 원본 화면이 backdrop으로 dim 처리되고 하단에서 시트 컴포넌트가 올라옴
3. **편집**: 시트 안에서 결정 (단일 선택형 옵션 묶음, 텍스트 입력 등)
4. **저장 또는 취소**: 1차 액션(저장) 또는 2차 액션(취소·외부 탭·핸들 드래그 다운) 중 하나
5. **원본 시각 갱신**: 저장 시 원본 화면에 결정이 반영된 시각 단서가 즉시 반영 (예: 카드의 인디케이터 변화)

### 구조
- `Sheet` 컴포넌트 본체 (`Backdrop` + `Handle` + `SheetHeader` + `SheetContent` + `SheetActions`)
- 시트 본체는 영역 표면색으로 원본 영역과 일관
- backdrop은 영역 ink 색상에 alpha 적용 — 원본 화면의 영역 정체성을 유지하면서 주의를 시트로 끌어올림

### 규칙
- 시트 내부도 영역 토큰 일관 (배경·rule·강조색 모두 영역 정의를 따름)
- 다짐 영역에서 사용 시: 시트 안의 정서 본문(다짐 문장 미리보기 등)은 §3.1 규칙에 따라 serif 변형 허용. 시스템 UI 텍스트(헤더, 옵션 라벨, 버튼)는 sans 일관
- 1차 액션(저장)과 2차 액션(취소)의 시각 위계 명확 — 1차는 영역 강조색 채움 버튼, 2차는 텍스트 액션 또는 보더 버튼
- 시트는 단일 결정에 집중 — 여러 결정이 한 시트에 섞이면 분리 후보

### 영역별 적용 사례
- **다짐**(`screen-affirmations-priority-edit`): 새 다짐 추가 직후 자동 노출 → 우선순위(높음/보통/낮음) 단일 선택 → 저장 시 카드의 점 인디케이터·라벨·정렬 갱신
...
| `screen-affirmations-priority-edit` | 영역 화면 (다짐) | 편집 시트 |
