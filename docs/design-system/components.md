...
- 변형:
  ...
  - `history-group.unacked` (PR 모니터, PRD-10 AC13): `history-item.*` 카드들을 같은 PR(있으면) 또는 같은 커밋(`head_sha`) 기준으로 묶는 한 단계 상위 컨테이너. 외곽은 `bg-white` + 좌측 3px `--accent` 보더 + 나머지 1px `--rule` 보더 + 작은 `rounded-md`. 그룹 헤더는 PR 키 정보(`repo · #N PR-title`) 또는 커밋 키 정보(`repo · branch @sha`), 그 아래 종합 상태(`통과 X · 실패 Y · CI N건`)와 최근 시각, 우측에 `미확인 N` 배지(`bg-[var(--accent)] text-white`). 그룹 본체는 헤더 아래 `border-t border-[var(--rule)]`로 구분되며, 그 안에 `history-item.unacked` / `history-item.acked` 행이 시각 역순으로 들어간다. 행 사이는 `border-t border-[var(--rule)]/60` 점진 구분. `pulse-glow` 옵션은 그룹 카드 단위에 적용 가능 — 푸시 진입한 항목이 속한 그룹 자체가 외곽 링 펄스를 받는다(5초 자동 해제).
