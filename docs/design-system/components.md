...
- 변형:
  ...
  - `history-item.unacked` (PR 모니터, PRD-10 AC11/12): surface + 좌측 3px `--accent`(인디고) 보더 + 나머지 1px `--rule` 보더 + 상태 원형 배지(성공 `--forest` / 실패 `--destructive`) + 외부 링크 칩 1개(런 — workflow_run마다 고유한 링크) + 명시적 "확인" 버튼(영역 강조색 채움). PR / 커밋 외부 링크는 row가 아니라 `history-group.unacked` 헤더에 위치한다(같은 그룹 안에서 PR URL은 동일, 대표 커밋은 그룹 키 또는 lead 항목). row 메타 라인은 짧은 commit SHA 텍스트(`commit xxxxxxx`)를 그대로 유지해 row 단위 커밋 식별이 가능하다. 옵션: `pulse-glow` 변형 — 푸시 진입 직후 카드만 `--accent-strong` 4~6% 알파 외곽 링이 펄스. 5초 후 자동 해제(자세한 동작은 `tokens.md` §1.11 마지막 항목).
  ...
  - `history-group.unacked` (PR 모니터, PRD-10 AC13): `history-item.*` 카드들을 같은 PR(있으면) 또는 같은 커밋(`head_sha`) 기준으로 묶는 한 단계 상위 컨테이너. 외곽은 `bg-white` + 좌측 3px `--accent` 보더 + 나머지 1px `--rule` 보더 + 작은 `rounded-md`. 그룹 헤더는 PR 키 정보(`repo · #N PR-title`) 또는 커밋 키 정보(`repo · branch @sha`), 그 아래 종합 상태(`통과 X · 실패 Y · CI N건`)와 최근 시각, **그 아래 외부 링크 칩(PR · 커밋)** — PR 그룹은 `PR` + 대표 커밋 칩, 커밋 전용 그룹은 커밋 칩만. 우측에 `미확인 N` 배지(`bg-[var(--accent)] text-white`)와 그 아래 `모두 확인` 보더 버튼(그룹 내 미확인 항목 일괄 ack 트리거). 그룹 본체는 헤더 아래 `border-t border-[var(--rule)]`로 구분되며, 그 안에 `history-item.unacked` / `history-item.acked` 행이 시각 역순으로 들어간다. 행 사이는 `border-t border-[var(--rule)]/60` 점진 구분. `pulse-glow` 옵션은 그룹 카드 단위에 적용 가능 — 푸시 진입한 항목이 속한 그룹 자체가 외곽 링 펄스를 받는다(5초 자동 해제).
  - `history-group.acked` (PR 모니터): 그룹 내 모든 항목이 확인된 상태. `bg-[var(--bg)]/60` + `border border-dashed border-[var(--rule)]` + `opacity-70`, 헤더만 표시되고 본체는 접힌 채로 둔다. 우측 배지는 `모두 확인`(중립 stone, 보더만). 본문 텍스트는 `line-through decoration-stone-300`.
...
- 인터랙션:
  ...
  - **선택적 destructive 액션**: 편집(edit) 모드의 시트에서 항목 삭제가 가능한 경우 1차 액션 아래에 외곽선(stroke-only) 형태의 destructive 버튼을 추가. 라벨 + 색은 `tokens.md` §1.10의 영역 `--destructive` 토큰을 사용한다. 새 항목 생성(create) 모드에는 노출하지 않는다.
...
| `Card.history-group.unacked` | PR 모니터 영역 전용 | §1.11 인디고 + 의미 토큰(forest/destructive) |
| `Card.history-group.acked` | PR 모니터 영역 전용 | §1.11 인디고 + stone(중립) |
