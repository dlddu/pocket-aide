...
- **사용자 여정**: 부분 정의됨. `docs/user-journeys/`에 2개 여정(`journey-writer-keyboard-edit.md`, `journey-reminder-widget-glance.md`) 작성됨. 다음 mockup의 "여정"은 여전히 `(미정의)`: screen-chat-text, screen-chat-voice, screen-scratchpad, screen-todo-personal, screen-todo-work, screen-routines, screen-shortcut-capture (총 7개).
...
- **시각화 대상**:
  - 여정: journey-reminder-widget-glance.md (S1 등록·우선순위, S4 탭 진입, S5 TTS 재생)
  - 가치: V4 (의도된 반복 노출)
...
## screen-widget.html
- **시각화 대상**:
  - 여정: journey-reminder-widget-glance.md (S2 통합 시야, S3 다짐 회전 갱신)
...
- **시각화 대상**:
  - 여정: journey-writer-keyboard-edit.md (S1 키보드 전환, S2 명령/자유 지시, S3 비동기 처리, S4 결과 반영)
