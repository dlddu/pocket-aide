...
//
// All tests are currently skipped because handler/auth.go has not yet been
// implemented. Remove the t.Skip() call in each test to activate it once the
// corresponding production code is in place.
//
...
func TestAuthHandler_Register_ReturnsCreated(t *testing.T) {
	t.Skip("TODO: 인증 기능 구현 후 활성화 — DLD-717")
	...
	_ = httptest.NewRecorder() // satisfy import until skip is removed
...
func TestAuthHandler_Me_WithoutToken_ReturnsUnauthorized(t *testing.T) {
	...
	// Arrange
	// e := echo.New()
	// authHandler := handler.NewAuthHandler(nil, "test-jwt-secret") // no DB needed
	// e.GET("/me", authHandler.Me, appmiddleware.JWT("test-jwt-secret"))
	//
	// req := httptest.NewRequest(http.MethodGet, "/me", nil)
	// rec := httptest.NewRecorder()
