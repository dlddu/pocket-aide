...
func TestRoutineHandler_List_DDayCalculation_IncludesDaysRemaining(t *testing.T) {
	...
	// Use dynamic dates: last_done_at = today - 4 days, so next_due_date = today + 3 (interval=7)
	...
	// next_due_date = lastDoneAt + 7 = today + 3 → d_day = 3
	if dDay != float64(3) {
...
// Scenario:
//
//	Seed: routine with interval_days=7, last_done_at= 14 days ago
...
func TestRoutineHandler_Complete_UpdatesLastDoneAndNextDue(t *testing.T) {
	...
	oldDate := today.AddDate(0, 0, -14).Format("2006-01-02")
	expectedNextDue := today.AddDate(0, 0, 7).Format("2006-01-02")

	// Create a routine with an old last_done_at
	payload := fmt.Sprintf(`{"name":"독서","interval_days":7,"last_done_at":"%s"}`, oldDate)
	createReq := httptest.NewRequest(http.MethodPost, "/routines", strings.NewReader(payload))
	createReq.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
	createReq.Header.Set(echo.HeaderAuthorization, token)
	createRec := httptest.NewRecorder()
	...
	if routine["last_done_at"] != todayStr {
		t.Errorf("expected last_done_at '%s' after complete, got %v", todayStr, routine["last_done_at"])
	}
