...
func TestRoutineHandler_List_DDayCalculation_IncludesDaysRemaining(t *testing.T) {
	...
	// Use dynamic dates: last_done_at = today - 4 days, interval = 7
	// → next_due_date = today + 3 → d_day = 3
	...
	if dDay != float64(3) {
		t.Errorf("expected d_day 3, got %v", dDay)
	}
}
...
// Scenario:
//
//	Seed: routine with interval_days=7, last_done_at="<13 days ago>"
...
func TestRoutineHandler_Complete_UpdatesLastDoneAndNextDue(t *testing.T) {
	...
	// Use dynamic dates to avoid date-dependent test failures
	today := time.Now().Truncate(24 * time.Hour)
	oldLastDone := today.AddDate(0, 0, -13).Format("2006-01-02")
	expectedToday := today.Format("2006-01-02")
	expectedNextDue := today.AddDate(0, 0, 7).Format("2006-01-02")

	// Create a routine with an old last_done_at
	payload := fmt.Sprintf(`{"name":"독서","interval_days":7,"last_done_at":"%s"}`, oldLastDone)
	createReq := httptest.NewRequest(http.MethodPost, "/routines", strings.NewReader(payload))
	createReq.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
	createReq.Header.Set(echo.HeaderAuthorization, token)
	createRec := httptest.NewRecorder()
	...
	if routine["last_done_at"] != expectedToday {
		t.Errorf("expected last_done_at '%s' after complete, got %v", expectedToday, routine["last_done_at"])
	}
